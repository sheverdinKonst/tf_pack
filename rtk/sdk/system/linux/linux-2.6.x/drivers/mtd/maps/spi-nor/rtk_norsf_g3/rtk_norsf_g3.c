...
#define SFNOR_BASE_ADDR		0x94000000
...
cache_type_t g_cpu_cache_type;


void cache_flush_spinor_area(void)
{
    if(g_cpu_cache_type == cache_l2)
    {
        dma_cache_inv(SFNOR_BASE_ADDR, 64*1024*1024);
    }else{
        __flush_cache_all();
    }
}
...
__init static int init_luna_nor_spi_map(void) {
    ...
    if(FAMILY_ID(chip_id) == RTL9310_FAMILY_ID)
    {
        g_cpu_cache_type = cache_l2;
    }
