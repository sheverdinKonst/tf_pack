...
/***********************************************
  *  Toshiba's ID Definition: MID+DID
  *  Manufacture ID (MID): KIOXIA (98h)
  *  Devide ID (DID)
  ***********************************************/
...
/************************************************
  *  KIOXIA's ID Definition: MID+DID+OID
  *  Manufacture ID (MID): KIOXIA (98h)
  *  Devide ID (DID)
  *  Organiztion ID (OID)
  ***********************************************/
#define MID_KIOXIA          (MID_TOSHIBA)
#define DID_TC58CVG0S3HRAIJ (0xE240)
#define DID_TC58CVG1S3HRAIJ (0xEB40)
...
toshiba_read_id(u32_t cs)
{
    ...
    // refactor ID for 0xMM00_DDDD, DDDD = DID (Toshiba) or DID+OID (KIOXIA)
    u32_t id = (ret&(0xFF<<24));
    if(((ret>>20)&0xF)==0xC) {   // Toshiba
        id |= ((ret>>16)&0xFF);
    } else {
        id |= ((ret>>8)&0xFFFF);
    }
...
probe_toshiba_spi_nand_chip(void)
{
    ...
    if(MID_TOSHIBA != ((rdid>>24)&0xFF)) return VZERO;

    u16_t did = rdid &0xFFFF;
    u32_t i;
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(toshiba_chip_info) ; i++){
        if((toshiba_chip_info[i].dev_id == did) || (toshiba_chip_info[i].dev_id == DEFAULT_DATA_BASE)){
            #if __DEVICE_REASSIGN
                #if __DEVICE_USING_QIO
...
toshiba_init_rest(spi_nand_flash_info_t * _spi_nand_info)
{
    ...
    if(((rdid>>24)!=MID_TOSHIBA) || ((rdid&0xFFFF)!=_spi_nand_info->dev_id))
      { return 0; }
